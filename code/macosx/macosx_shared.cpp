...
static void MAC_MicroSleep(int us)
{
	// FIXME: !!!
}
...
static int Mac_RunProcess( char** argv )
{
...
void Mac_HardRebootHandler( int argc, char** argv )
{
...
static const char* Mac_GetExeName(const char* path)
{
...
void Mac_TrackParentProcess()
{
	...
	lin_hasParent = strcmp(Mac_GetExeName(cmdLine), Mac_GetExeName(q_argv[0])) == 0;
}
...
static int Sys_GetProcessUptime(pid_t pid)
{
	...
	return (int)(now.tv_sec - kp.pbi_start_tvsec);
}
...
void Mac_QueEvent( int time, sysEventType_t type, int value, int value2, int ptrLength, void *ptr )
{
...
sysEvent_t Sys_GetEvent()
{
	...
	const char* s = Mac_ConsoleInput();
	if ( s ) {
		const int slen = strlen( s );
		const int blen = slen + 1;
		char* b = (char*)Z_Malloc( blen );
		Q_strncpyz( b, s, blen );
		Mac_QueEvent( 0, SE_CONSOLE, 0, 0, slen, b );
	}
	...
	if ( Sys_GetPacket( &adr, &netmsg ) ) {
		...
		*buf = adr;
		...
		Mac_QueEvent( 0, SE_PACKET, 0, 0, len, buf );
	}
