...
static void sdl_Key( const SDL_KeyboardEvent* event, qbool down )
{
	...
	if (key >= 0)
		Mac_QueEvent(Sys_Milliseconds(), SE_KEY, key, down, 0, NULL);

	if (down && key == K_BACKSPACE)
		Mac_QueEvent(Sys_Milliseconds(), SE_CHAR, 8, 0, 0, NULL);

	// ctrl+v
	if (down && key == 'v' && (event->keysym.mod & KMOD_CTRL) != 0)
		Mac_QueEvent(Sys_Milliseconds(), SE_CHAR, 22, 0, 0, NULL);
}
...
static void sdl_Text( const SDL_TextInputEvent* event )
{
	...
	if (key >= 0 && key <= 0x7F)
		Mac_QueEvent(Sys_Milliseconds(), SE_CHAR, (int)key, 0, 0, NULL);
}
...
static void sdl_MouseMotion( const SDL_MouseMotionEvent* event )
{
	...
	Mac_QueEvent(Sys_Milliseconds(), SE_MOUSE, event->xrel, event->yrel, 0, NULL);
}
...
static void sdl_MouseButton( const SDL_MouseButtonEvent* event, qbool down )
{
	...
	Mac_QueEvent(Sys_Milliseconds(), SE_KEY, mouseButtons[button][1], down, 0, NULL);
}
...
static void sdl_MouseWheel( const SDL_MouseWheelEvent* event )
{
	...
	Mac_QueEvent(Sys_Milliseconds(), SE_KEY, key, qtrue,  0, NULL);
	Mac_QueEvent(Sys_Milliseconds(), SE_KEY, key, qfalse, 0, NULL);
}
