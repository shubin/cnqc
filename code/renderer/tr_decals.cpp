...
static void R_BoxSurfaces_r( mnode_t *node, vec3_t mins, vec3_t maxs, surfaceType_t **list, int listsize, int *listlength, vec3_t dir ) {
	...
	while ( c-- ) {
		...
		// extra check for surfaces to avoid list overflows
			...
			if ( s == 1 || s == 2 ) {
				surf->vcBSP = tr.viewCount;
			} else if ( DotProduct( ( ( srfSurfaceFace_t * )surf->data )->plane.normal, dir ) > ANGLE_CUTOFF ) {
...
{
	static surfaceType_t *surfaces[MAX_SURFACES];
	...
	dists[numPoints] = DotProduct( normals[numPoints], points[0] ) - 100;
	VectorCopy( projectionDir, normals[numPoints + 1] );
	VectorInverse( normals[numPoints + 1] );
	dists[numPoints + 1] = DotProduct( normals[numPoints + 1], points[0] ) - 60;
	numPlanes = numPoints + 2;

	numsurfaces = 0;
#if defined( QC )
	R_BoxSurfaces_r( tr.world->nodes, mins, maxs, surfaces, ARRAY_LEN( surfaces ), &numsurfaces, projectionDir );
#else // QC
		...
		if ( *surfaces[i] == SF_GRID ) {
			...
			for ( m = 0; m < cv->height - 1; m++ ) {
				for ( n = 0; n < cv->width - 1; n++ ) {
					...
					if ( DotProduct( normal, projectionDir ) < ANGLE_CUTOFF ) {
			...
			if ( DotProduct( surf->plane.normal, projectionDir ) > -0.1 ) {
				continue;
			}
